...
final class Dispatcher {
     ...
     */
    public static function dispatch(?string $requested_url = null, $default = null) {
        Flash::init();
...
// Dispatch
    ...
    public static function getController() {
        ...
        if (isset(self::$params[0]) && self::$params[0] == 'plugin') {
            $loadedPlugins = Plugin::$plugins;
            if (count(self::$params) < 2) {
                unset(self::$params[0]);
            } elseif (isset(self::$params[1]) && !isset($loadedPlugins[self::$params[1]])) {
                unset(self::$params[0]);
                unset(self::$params[1]);
            }
        }

        return self::$params[0] ?? DEFAULT_CONTROLLER;
    }
    ...
    public static function getAction() {
        return self::$params[1] ?? DEFAULT_ACTION;
    }
    ...
    public static function getStatus($key = null) {
        return ($key === null) ? self::$status : ( self::$status[$key] ?? null);
    }
    ...
    public static function executeAction($controller, $action, $params) {
        ...
        $controllerClass = Inflector::camelize($controller);
        ...
        if (!$controller instanceof Controller) {
            throw new Exception("Class '{$controllerClassName}' does not extends Controller class!");
        }
...
class Record {
     ...
     */
    final public static function connection(\PDO $connection) {
     ...
     */
    final public static function logQuery(string $sql) {
     ...
     */
    final public static function query(string $sql, array $values = []) {
        self::logQuery($sql);
     ...
     */
    final public static function tableNameFromClassName(string $className) {
        try {
            if (class_exists($className) && defined($className . '::TABLE_NAME')) {
                return TABLE_PREFIX . constant($className . '::TABLE_NAME');
            }
        } catch (Exception $e) {
            return TABLE_PREFIX . Inflector::underscore($className);
        }
    }
        ...
        if (empty($this->id)) {
            ...
            foreach ($columns as $column) {
                ...
                if (!empty($this->$column) || is_numeric($this->$column)) { // Do include 0 as value
                    $valueOf[$column] = self::$__CONN__->quote($this->$column);
                    ...
                    if (self::$__CONN__->getAttribute(PDO::ATTR_DRIVER_NAME) != 'sqlite') {
                        $valueOf[$column] = 'DEFAULT';
                    }
                }
            }

            $sql = 'INSERT INTO ' . self::tableNameFromClassName(get_class($this)) . ' ('
                    . implode(', ', array_keys($valueOf)) . ') VALUES (' . implode(', ', array_values($valueOf)) . ')';
            ...
            foreach ($columns as $column) {
                if (!empty($this->$column) || is_numeric($this->$column)) { // Do include 0 as value
                    $valueOf[$column] = $column . '=' . self::$__CONN__->quote($this->$column);
                } elseif (isset($this->$column)) { // Properly fallback to the default column value instead of relying on an empty string
                    // SQLite can't handle the DEFAULT value
                    if (self::$__CONN__->getAttribute(PDO::ATTR_DRIVER_NAME) != 'sqlite') {
                        $valueOf[$column] = $column . '=DEFAULT';
                    } else {
                        //Since DEFAULT values don't work in SQLite empty strings should be passed explicitly
                        $valueOf[$column] = $column . "=''";
                    }
                }
            }
            ...
            $sql = 'UPDATE ' . self::tableNameFromClassName(get_class($this)) . ' SET '
                    . implode(', ', $valueOf) . ' WHERE id = ' . $this->id;
            $return = self::$__CONN__->exec($sql) !== false;
     ...
     *
     * @param string $className    The classname of the record that should be inserted.
     * @param array $data           An array of key/value pairs to be inserted.
     * @return boolean              Returns true when successful.
     */
    public static function insert(string $className, array $data) {
        $keys = [];
        $values = [];
        ...
        $sql = 'INSERT INTO ' . self::tableNameFromClassName($className) . ' (' . implode(', ', $keys) . ') VALUES (' . implode(', ', $values) . ')';
     ...
     *
     * @param string $className    The classname of the record to be updated.
     ...
     * @return <type>
     */
    public static function update(string $className, array $data, string $where, array $values = []) {
        $setters = [];
        ...
        $sql = 'UPDATE ' . self::tableNameFromClassName($className) . ' SET ' . implode(', ', $setters) . ' WHERE ' . $where;
     ...
     *
     * @param string $className    The classname for the record to be deleted.
     * @param string $where         An SQL WHERE clause to specify a specific record.
     * @param array $values         An array of values if this is a prepared statement.
     * @return boolean              True when the delete was successful.
     */
    public static function deleteWhere(string $className, string $where, array $values = []) {
        $sql = 'DELETE FROM ' . self::tableNameFromClassName($className) . ' WHERE ' . $where;
        self::logQuery($sql);
        $stmt = self::$__CONN__->prepare($sql);
        var_dump($stmt->execute($values),$stmt->errorInfo());
        return $stmt->execute($values);
    }
     ...
     *
     * @param string $className    The classname to be returned.
     ...
     */
    public static function existsIn(string $className, string $where, array $values = []) {
        $sql = 'SELECT EXISTS(SELECT 1 FROM ' . self::tableNameFromClassName($className) . ($where ? ' WHERE ' . $where : '') . ' LIMIT 1)';
    ...
    public static function find(array $options = []) {
        ...
        // Collect attributes
        $ses = trim($options['select'] ?? '');
        $frs = trim($options['from'] ?? '');
        $jos = trim($options['joins'] ?? '');
        $whs = trim($options['where'] ?? '');
        $gbs = trim($options['group'] ?? '');
        $has = trim($options['having'] ?? '');
        $obs = trim($options['order'] ?? '');
        $lis = (int) ($options['limit'] ?? 0);
        $ofs = (int) ($options['offset'] ?? 0);
        ...
        $groupBy = empty($gbs) ? '' : "GROUP BY $gbs";
        $having = empty($has) ? '' : "HAVING $has";
        $orderBy = empty($obs) ? '' : "ORDER BY $obs";
        $limit = $lis > 0 ? "LIMIT $lis" : '';
        $offset = $ofs > 0 ? "OFFSET $ofs" : '';

        // Build the query
        $sql = trim("$select $from $joins $where $groupBy $having $orderBy $limit $offset");

        // Run query
        $objects = self::findBySql(trim($sql), $values);

        return ($single) ? (!empty($objects) ? $objects[0] : false) : $objects;
    }

    private static function findBySql(string $sql, array $values = []) {
     ...
     */
    public static function findByIdFrom(string $className, int $id) {
     ...
     * @return                      Single object
     */
    public static function findOne(array $options = []) {
        $options['limit'] = 1;
     ...
     * @return Record               A record instance or false on failure.
     */
    public static function findOneFrom(string $className, string $where = '', array $values = []) {
     ...
     */
    public static function findAllFrom(string $className, string $where = '', array $values = []) {
        if ($where) {
     ...
     */
    public static function countFrom(string $className, string $where = '', array $values = []) {
        $sql = 'SELECT COUNT(*) AS nb_rows FROM ' . self::tableNameFromClassName($className) . ($where ? ' WHERE ' . $where : '');
...
class View {
    ...
    private string $file;           // String of template file
    private array $vars = []; // Array of template variables
...
class Controller {
     ...
     */
    public function assignToLayout(string $var, $value = null) {
        if (is_array($var)) {
    ...
    public function render(string $view, array $vars = []) {
        if ($this->layout) {
            $this->layoutVars['content_for_layout'] = new View($view, $vars);
            return new View('../layouts/' . $this->layout, $this->layoutVars);
        } else {
     ...
     * @param <type> $exit
     */
    public function display(string $view, array $vars = [], bool $exit = true) {
     ...
     *
     * @param mixed $data The data being encoded.
...
final class Observer {
     ...
     */
    public static function observe(string $eventName, $callback) {
        if (!isset(self::$events[$eventName])) {
            self::$events[$eventName] = [];
        }

        self::$events[$eventName][$callback] = $callback;
    }
     ...
     * @param string $callback      The name of the function handling the event.
     */
    public static function stopObserving(string $eventName, $callback) {
        if (isset(self::$events[$eventName][$callback])) {
            unset(self::$events[$eventName][$callback]);
        }
    }
     ...
     */
    public static function clearObservers(string $eventName) {
        self::$events[$eventName] = [];
    }
     ...
     */
    public static function getObserverList(string $eventName) {
    ...
    public static function notify($eventName) {
        ...
        foreach (self::getObserverList($eventName) as $callback) {
...
class AutoLoader {
     ...
     * </code>
     *
     * @param mixed $className Classname or array of classname/path pairs.
     * @param mixed $file       Full path to the file that contains $class_name.
     */
    public static function addFile(string $className, $file = null) {
        if ($file == null && is_array($className)) {
            self::$files = array_merge(self::$files, $className);
        } else {
    ...
    public static function load(string $className) {
        if (isset(self::$files[$className])) {
            ...
            foreach (self::$folders as $folder) {
                ...
                $file = $folder . DIRECTORY_SEPARATOR . str_replace('\\', DIRECTORY_SEPARATOR, $className) . '.php';
                if (file_exists($file)) {
                    require $file;
                    return;
                }
        ...
        throw new Exception("AutoLoader could not find file for '{$className}'.");
    }
...
final class Flash {
    ...
    private static $_flashstore = []; // Data that prevous page left in the Flash
    ...
    public static function get(string $var, $default=null) {
        return self::$_flashstore[$var] ?? $default;
    }
     ...
     * @param mixed $value  Variable value
     */
    public static function setNow(string $var, $value) {
...
function use_helper() {
    ...
    foreach ($helpers as $helper) {
        ...
        $helperFile = HELPER_PATH . DIRECTORY_SEPARATOR . $helper . '.php';
...
function use_model() {
    ...
    foreach ($models as $model) {
        ...
        $modelFile = APP_PATH . DIRECTORY_SEPARATOR . 'models' . DIRECTORY_SEPARATOR . $model . '.php';
 ...
 */
function html_encode(string $string) {
 ...
 */
function html_decode(string $string) {
...
// Note that you have to handle splits with \n, \r, and \t later since they *are* allowed in some inputs
    ...
    $ra = ['javascript', 'vbscript', 'expression', 'applet', 'meta', 'xml', 'blink', 'style',
 ...
 * @return <type>
 */
function cleanArrayXSS(array $ar) {
    $ret = [];
 ...
 * @return string
 */
function jsEscape(string $value) {
    return strtr($value, [
        "'" => '\\\'',
        '"' => '\"',
        '\\' => '\\\\',
 ...
 */
function pageNotFound(?string $url = null) {
 ...
 */
function getContentFromUrl(string $url, int $flags = 0, $context = false) {
    ...
    if (ini_get('allow_url_fopen') && function_exists('file_get_contents')) {
        if ($context === false){
            $context = stream_context_create(['http' => ['timeout' => CHECK_TIMEOUT]]);
        }
 ...
 */
function framework_exception_handler(Throwable $e) {
    if (!DEBUG){
        pageNotFound();
    }
    ...
    $dispatcherStatus = Dispatcher::getStatus();
    $dispatcherStatus['request method'] = get_request_method();
    debug_table($dispatcherStatus, 'Dispatcher status');
    if (!empty($_GET)){
 ...
 * @param <type> $label
 * @param <type> $keyLabel
 * @param <type> $valueLabel
 */
function debug_table(array $array, string $label, string $keyLabel = 'Variable', string $valueLabel = 'Value') {
    ...
    echo '<tr><td style="border-right: 1px solid #000; border-bottom: 1px solid #000;">' . $keyLabel . '</td>' .
    '<td style="border-bottom: 1px solid #000;">' . $valueLabel . '</td></tr></thead>';
