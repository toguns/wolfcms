...
class FileManagerController extends PluginController {
    ...
    public function browse() {
        ...
        $this->display('file_manager/views/index', [
...
// browse
    ...
    public function view() {
        ...
        $this->display('file_manager/views/view', [
    ...
    public function save() {
        ...
        if (isset($_POST['csrf_token'])) {
            ...
            if (!SecureToken::validateToken($csrf_token, BASE_URL . 'plugin/file_manager/save/' . $data['name'])) {
                ...
                redirect(getUrl('plugin/file_manager/view/' . $data['name']));
            }
        } else {
        ...
        if (file_exists($file)) {
            ...
            if (file_put_contents($file, $data['content'])) {
                Flash::set('success', __('File :name has been created with success!', [':name' => $data['name']]));
            } else {
        ...
        if (isset($_POST['commit'])) {
            redirect(getUrl('plugin/file_manager/browse/' . substr($data['name'], 0, strrpos($data['name'], '/'))));
        } else {
            redirect(getUrl('plugin/file_manager/view/' . $data['name'] . (endsWith($data['name'], URL_SUFFIX) ? '?has_url_suffix=1' : '')));
        }
    }
    ...
    public function create_file() {
        if (!AuthUser::hasPermission('file_manager_mkfile')) {
            ...
            redirect(getUrl('plugin/file_manager/browse/'));
        }
        ...
        if (file_put_contents($file, '') !== false) {
            ...
            Flash::set('error', __('File :name has not been created!', [':name' => $filename]));
        }
        redirect(getUrl('plugin/file_manager/browse/' . $path));
    }
    ...
    public function create_directory() {
        ...
        if (mkdir($dir)) {
            ...
            Flash::set('error', __('Directory :name has not been created!', [':name' => $dirname]));
        }
        redirect(getUrl('plugin/file_manager/browse/' . $path));
    }
    ...
    public function delete() {
        ...
        redirect(getUrl('plugin/file_manager/browse/' . $paths));
    }
    ...
    public function upload() {
        ...
        $ext_arr = ['php', 'php3', 'php4', 'php5', 'php6', 'php7','inc', 'pht', 'phtml'];
    ...
    private function _listFiles() {
        if (is_dir($this->fullpath)) {
            ...
            foreach ($root as $cur) {
                ...
                if ($cur->isDir()) {
                    $object->link = '<a href="' . getUrl('plugin/file_manager/browse/' . $this->path . $object->name) . '">' . $object->name . '</a>';
                } else {
                    $object->link = '<a href="' . getUrl('plugin/file_manager/view/' . $this->path . $object->name . (endsWith($object->name, URL_SUFFIX) ? '?has_url_suffix=1' : '')) . '">' . $object->name . '</a>';
                }
    ...
    private function _getPermissions($perms) {
        ...
        return [$info, substr(sprintf('%o', $perms), -4, 4)]; // (perm, chmod)
    }
    ...
    public function settings() {
        ...
        $this->display('file_manager/views/settings', ['settings' => $settings]);
    }
    ...
    public function settings_save() {
        ...
        if (!isset($_POST['settings'])) {
            ...
            redirect(getUrl('plugin/file_manager/settings'));
        } else {
