<?php

/**
 * Wolf CMS multi_lang plugin language file
 *
 * @package Translations
 */
return [
    'Do you want to create a translated version of a page as a tab of the same page or as a copy of the page in a language specific subtree? (i.e. Home->nl->About as a Dutch translation of Home->About)' => '如果您希望新增一個已經翻譯過的頁面,但是希望顯示於同一個頁面不同頁籤,並且可以複製特定語系的頁面在子目錄下嗎?(例如:Home->nl->關於中文翻譯-  Home->About)',
    'Documentation' => '說明文件',
    'General' => '一般',
    'Get the language preference from the HTTP header (default), the uri (/nl/about.html for the Dutch version of about.html) or from the stored preference of a logged in user.' => 'Get the language preference from the HTTP header (default), the uri (/nl/about.html for the Dutch version of about.html) or from the stored preference of a logged in user.',
    'HTTP_ACCEPT_LANG header' => 'HTTP_ACCEPT_LANG 的表頭',
    'Language source' => '語系來源',
    'Multiple Language Documentation' => '多重語系說明文件',
    'Multiple Language Settings' => '多重語系設定',
    'Multiple Languages' => '多重語系',
    'Provides language specific content when available based on user preferences.' => '提供特定語系內容,此功能將會因為使用者設定後開啟',
    'Save' => '儲存',
    'Settings' => '網站設定',
    'Style' => '風格',
    'The settings have been updated.' => 'The settings have been updated.',
    'Translations as page copy' => 'Translations as page copy',
    'Translations as tab' => 'Translations as tab',
    'URI' => '網址',
    'Wolf CMS user preferences' => 'Wolf CMS 使用者介面',
    'You have modified this page.  If you navigate away from this page without first saving your data, the changes will be lost.' => 'You have modified this page.  If you navigate away from this page without first saving your data, the changes will be lost.'
];
