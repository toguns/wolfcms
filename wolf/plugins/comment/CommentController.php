...
class CommentController extends PluginController {
    ...
    function index($page = 0) {
        $this->display('comment/views/index', [
    ...
    function settings() {
        ...
        $settings = ['approve' => $tmp['auto_approve_comment'],
    ...
    function edit($id = null) {
        if (is_null($id))
            redirect(getUrl('plugin/comment'));
        ...
        // display things...
        $this->display('comment/views/edit', [
            'action' => 'edit',
            'comment' => $comment
        ]);
    }
    ...
    function _edit($id) {
        ...
        if (!$comment->save()) {
            ...
            redirect(getUrl('plugin/comment/edit/' . $id));
        } else {
    ...
    function approve($id) {
        ...
        redirect(getUrl('plugin/comment/moderation'));
    }
    ...
    function save() {
        ...
        $settings = ['auto_approve_comment' => $approve,
            'use_captcha' => $captcha,
            'rowspage' => $rowspage,
            'numlabel' => $numlabel
        ];
        ...
        redirect(getUrl('plugin/comment/settings'));
    }

    function moderation($page = 0) {
        $this->display('comment/views/moderation', [
            'comments' => Comment::findAll(),
            'page' => $page
        ]);
    }
