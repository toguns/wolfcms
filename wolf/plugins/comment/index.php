...
function comment_on_page_saved($page) {
    ...
    Record::update('Page', ['comment_status' => $status], 'id = ?', [$page->id]);
}
...
function comments_count_moderatable() {
    return (int) count(Comment::find(['where' => 'is_approved=0']));
}
...
function comment_save(&$page) {
    ...
    if (Plugin::isEnabled('statistics_api')) {
        $event = ['event_type' => 'comment_added', // simple event type identifier
