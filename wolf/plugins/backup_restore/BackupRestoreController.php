...
class BackupRestoreController extends PluginController {
    ...
    function settings() {
        $this->display('backup_restore/views/settings', ['settings' => Plugin::getAllSettings('backup_restore')]);
    }
    ...
    function save() {
        ...
        redirect(getUrl('plugin/backup_restore/settings'));
    }
    ...
    function backup() {
        ...
        $cdata_fields = ['title', 'content', 'content_html'];
    ...
    function restore() {
        if (isset($_POST['action']) && $_POST['action'] == 'restore') {
            ...
            $this->display('backup_restore/views/restore', ['settings' => Plugin::getAllSettings('backup_restore')]);
        }
    }
    ...
    function _restore($fileData) {
        ...
        if (false === $xml) {
            ...
            Flash::set('error', 'An error occurred with the XML backup file: :error', [':error' => $errors]);
            redirect(getUrl('plugin/backup_restore'));
        }
        ...
        foreach ($tablenames as $tablename) {
            ...
            if (array_key_exists($container, $xml) && count($xml->$container->$tablename) > 0) {
                ...
                if (false === $__CMS_CONN__->exec($sql)) {
                    Flash::set('error', __('Unable to truncate current table :tablename.', [':tablename' => $tablename]));
                    redirect(getUrl('plugin/backup_restore'));
                }
                ...
                foreach ($xml->$container->$tablename as $element) {
                    ...
                    if ($__CMS_CONN__->exec($sql) === false) {
                        Flash::set('error', __('Unable to reconstruct table :tablename.', [':tablename' => $tablename]));
                        redirect(getUrl('plugin/backup_restore'));
                    }
                }
            }
        ...
        if (($settings['restorefiles'] == '1') && array_key_exists('files', $xml)) {
            ...
            foreach ($xml->files->directory as $obj) {
                ...
                if (!file_exists($name)) {
                    if (mkdir($name, 0777, true) === false) {
                        Flash::set('error', __('Unable to create directory :name.', [':name' => dirname($obj->name)]));
                        redirect(getUrl('plugin/backup_restore'));
                    }
                }
            ...
            foreach ($xml->files->file as $obj) {
                ...
                if (file_put_contents($name, base64_decode($obj->content)) === false) {
                    Flash::set('error', __('Unable to restore file :name.', [':name' => $obj->name]));
                    redirect(getUrl('plugin/backup_restore'));
                }
                $this->_restore_attributes($name, $obj);
            }
                ...
                if (!$obj->isDot()) {
                    if ($obj->isDir()) {
                        ...
                        if (rmdir($obj->getPathname()) === false) {
                            Flash::set('error', __('Unable to delete directory :name.', [':name' => substr($obj->getPathname(), strlen($basedir . '/'))]));
                            redirect(getUrl('plugin/backup_restore'));
                        }
                    } else {
                        if (unlink($obj->getPathname()) === false) {
                            Flash::set('error', __('Unable to delete file :name.', [':name' => substr($obj->getPathname(), strlen($basedir . '/'))]));
                            redirect(getUrl('plugin/backup_restore'));
                        }
                    }
                }
    ...
    function _restore_attributes($name, $obj) {
        ...
        if (array_key_exists('mode', $obj)) {
            if (chmod($name, $obj->mode) === false) {
                Flash::set('error', __('Unable to restore attributes for :name.', [':name' => $obj->name]));
                redirect(getUrl('plugin/backup_restore'));
            }
        }
        ...
        if (array_key_exists('mtime', $obj)) {
            ...
            if (touch($name, $mtime) === false) {
                Flash::set('error', __('Unable to restore modification date for :name.', [':name' => $obj->name]));
                redirect(getUrl('plugin/backup_restore'));
            }
        }
    }
