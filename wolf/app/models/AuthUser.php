...
class AuthUser {
    ...
    static protected bool $isLoggedIn = false;
    static protected $userId = false;
    static protected bool $isAdmin = false;
    ...
    static private final function setInfos(Record $user) {
        ...
        self::$isAdmin = self::hasPermission('administrator');
    }
    ...
    static public final function getUserName() {
        return self::$record->username ?? false;
    }
     ...
     */
    static public final function hasPermission(?string $permissions) {
        if ($permissions === null) {
            return true;
        }
     ...
     */
    static public final function hasRole(string $roles) {
        if (self::getId() == 1) {
            return true;
        }

        foreach (explode(',', $roles) as $role) {
            if (in_array(trim($role), self::$roles)) {
                return true;
            }
        }
     ...
     * @return <type> 
     */
    static public final function forceLogin(string $username, bool $setCookie = false) {
        return self::login($username, null, $setCookie, false);
    }
     ...
     * @param boolean $setCookie   Set a "remember me" cookie? Defaults to false.
     * @return boolean              Returns true when successful, otherwise false.
     */
    static public final function login(string $username, string $password, bool $setCookie = false, bool $validatePassword = true) {
        self::logout();
        ...
        if ($user instanceof User && (false === $validatePassword || self::validatePassword($user, $password))) {
     ...
     */
    static private final function challengeCookie(string $cookie) {
     ...
     */
    static private final function explodeCookie(string $cookie) {
     ...
     */
    static public final function generateSalt(int $max = 32) {
        use_helper('Hash');
        $hash = new Crypt_Hash('sha256');
     ...
     * @return <type>
     */
    static public final function generateHashedPassword(string $password, string $salt) {
        use_helper('Hash');
     ...
     */
    static public final function validatePassword(User $user, string $pwd) {
        if (empty($user->salt)) {
            // Pre 070 method
