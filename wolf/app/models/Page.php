...
class Page extends Node {
     ...
     */
    public function childrenCount($args = null, $value = [], $include_hidden = false) {
     ...
     */
    public function children($args = null, $value = [], $include_hidden = false) {
    ...
    public function _executeLayout() {
        ...
        $stmt->execute([':layout_id' => $this->_getLayoutId()]);
    ...
    public function setTags($tags) {
        ...
        if (count($tags) == 0) {
            ...
            foreach ($current_tags as $tag)
                Record::update('Tag', ['count' => 'count - 1'], 'name = :tag_name', [':tag_name' => $tag]);

            return Record::deleteWhere('PageTag', 'page_id = :page_id', [':page_id' => $this->id]);
        } else {
            ...
            foreach ($new_tags as $index => $tag_name) {
                if (!empty($tag_name)) {
                    ...
                    if (!$tag = Tag::findByName($tag_name))
                        $tag = new Tag(['name' => trim($tag_name)]);

                    $tag->count++;
                    $tag->save();

                    // create the relation between the page and the tag
                    $tag = new PageTag(['page_id' => $this->id, 'tag_id' => $tag->id]);
                    $tag->save();
                }
            }
            ...
            foreach ($old_tags as $index => $tag_name) {
                ...
                Record::deleteWhere('PageTag', 'page_id = :page_id AND tag_id = :tag_id', [':page_id' => $this->id, ':tag_id' => $tag->id]);
                $tag->count--;
                $tag->save();
            }
        }
     ...
     */
    public static function findByPath(string $path, $all = false) {
        $path = trim($path, '/');
        $hasBehavior = false;

        // adding the home root
        $slugs = array_merge([''], explode_path($path));
        $url = '';
    ...
    private static function findBySql($sql, $values = null) {
        ...
        while ($page = $stmt->fetchObject('Page')) {
            ...
            if (!empty($parent->behavior_id)) {
                // will return Page by default (if not found!)
                $pageClass = Behavior::loadPageHack($parent->behavior_id);
            }

            // create the object page
            $page = new $pageClass($page, $parent);
            $page->part = self::getParts($page->id);
            $objects[] = $page;
        }
    ...
    public static function findByBehaviour($name, $parent_id = false) {
        if ($parent_id !== false && is_int($parent_id)) {
            return self::findOne([
                        ...
                        'values' => [':behavior_id' => $name, ':parent_id' => (int) $parent_id]
            ]);
        } else {
            return self::findOne([
                        'where' => 'behavior_id = :behavior_id',
                        'values' => [':behavior_id' => $name]
            ]);
        }
    }
    ...
    public static function hasChildren($id) {
        return (boolean) self::countFrom('Page', 'parent_id = :parent_id', [':parent_id' => (int) $id]);
    }
    ...
    public static function cloneTree($page, $parent_id) {
        ...
        if (count($pageTags)) {
            foreach ($pageTags as $tagId => $tagName) {
                // create the relation between the page and the tag
                $tag = new PageTag(['page_id' => $clone->id, 'tag_id' => $tagId]);
                $tag->save();
            }
        }
    ...
    public static function getParts(int $pageId) {
