...
function url_start_with(string $url) {
    $url = trim($url, '/');
...
function main(string $path) {
    ...
    if (!USE_MOD_REWRITE && strpos($path, '?') !== false) {
        $_GET = []; // empty $_GET array since we're going to rebuild it
        list($path, $get_var) = explode('?', $path);
        $explodedGet = explode('&', $get_var);

        if (count($explodedGet)) {
            foreach ($explodedGet as $get) {
    ...
    if (Dispatcher::hasRoute($path)) {
        ...
        foreach (Observer::getObserverList('page_requested') as $callback) {
            $path = call_user_func_array($callback, [&$path]);
        }
