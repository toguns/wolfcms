...
class PageController extends Controller {
    ...
    public function add($parent_id = 1) {
        ...
        if (empty($page_parts)) {
            ...
            $big_sister = Record::findOneFrom('Page', 'parent_id=? ORDER BY id DESC', [$parent_id]);
            if ($big_sister) {
                // Get list of parts create the same for the new little sister
                $big_sister_parts = Record::findAllFrom('PagePart', 'page_id=? ORDER BY id', [$big_sister->id]);
                $page_parts = [];
                foreach ($big_sister_parts as $parts) {
                    $page_parts[] = new PagePart([
                        'name' => $parts->name,
                        'filter_id' => Setting::get('default_filter_id')
                    ]);
                }
            } else {
                $page_parts = [new PagePart(['filter_id' => Setting::get('default_filter_id')])];
            }
        }
    ...
    public function addPart() {
        ...
        $data = isset($_POST['part']) ? $_POST['part'] : [];
        ...
        if ($id > 1) {
            ...
            if ($page = Record::findByIdFrom('Page', $id)) {
                ...
                if ($page->delete()) {
                    ...
                    Flash::set('success', __('MSG_PAGE_DELETED_SUCCESS', [':title' => $page->title]));
                } else
                    Flash::set('error', __('Page :title has not been deleted!', [':title' => $page->title]));
            } else
                Flash::set('error', __('Page not found!'));
        } else
    ...
    function children($parent_id, $level, $return = false) {
        $expanded_rows = isset($_COOKIE['expanded_rows']) ? explode(',', $_COOKIE['expanded_rows']) : [];
        ...
        $content = new View('page/children', [
            'childrens' => $childrens,
            'level' => $level + 1,
        ]);
    ...
    function copy() {
        ...
        $newData = [$new_root_id,
            getUrl('page/edit/' . $new_root_id),
            $page->title(),
            $page->slug(),
            $newUrl,
            getUrl('page/add', $new_root_id),
            getUrl('page/delete/' . $new_root_id . '?csrf_token=' . SecureToken::generateToken(BASE_URL . 'page/delete/' . $new_root_id))];
    ...
    private function _getPartView($index = 1, $name = '', $filter_id = '', $content = '') {
        $page_part = new PagePart([
            'name' => $name,
            'filter_id' => $filter_id,
            'content' => $content]
        );

        return $this->render('page/part_edit', [
                    'index' => $index,
                    'page_part' => $page_part
        ]);
    }
    ...
    private function _store($action, $id = false) {
        ...
        if (empty($data['slug']) && $id != '1') {
            ...
            if ($data['slug'] == ADMIN_DIR) {
                $errors[] = __('You cannot have a slug named :slug!', [':slug' => ADMIN_DIR]);
            }
            ...
            if (($id != '1' && (!Validate::slug($data['slug']) || empty($data['slug']))) || ($id == '1' && !empty($data['slug']))) {
                $errors[] = __('Illegal value for :fieldname field!', [':fieldname' => 'slug']);
            }
            if (Record::existsIn('Page', 'parent_id = :parent_id AND slug = :slug AND id <> :id', [':parent_id' => $data['parent_id'], ':slug' => $data['slug'], ':id' => $id])) {
                $errors[] = __('Page with slug <b>:slug</b> already exists!', [':slug' => $data['slug']]);
            }
        }

        // Check all numerical fields for a page
        $fields = ['parent_id', 'layout_id', 'needs_login'];
        foreach ($fields as $field) {
            if (!Validate::digit($data[$field])) {
                $errors[] = __('Illegal value for :fieldname field!', [':fieldname' => $field]);
            }
        }

        // Check all date fields for a page
        $fields = ['created_on', 'published_on', 'valid_until'];
        foreach ($fields as $field) {
            if (isset($data[$field])) {
        ...
        $fields = ['created_on_time', 'published_on_time', 'valid_until_time'];
        foreach ($fields as $field) {
            if (isset($data[$field])) {
                $data[$field] = trim($data[$field]);
        ...
        foreach ($fields as $field) {
            use_helper('Kses');
            $data[$field] = kses(trim($data[$field]), []);
            /*
        ...
        if (!empty($data['behaviour_id']) && !Validate::slug($data['behaviour_id'])) {
            $errors[] = __('Illegal value for :fieldname field!', [':fieldname' => 'behaviour_id']);
        }
        ...
        if (Setting::get('allow_html_title') == 'off') {
            use_helper('Kses');
            $data['title'] = kses(trim($data['title']), []);
        }
        ...
        if (isset($_POST['commit'])) {
            ...
            redirect(getUrl('page/edit/' . $page->id));
        }
    }
