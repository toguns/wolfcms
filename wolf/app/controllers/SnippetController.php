...
class SnippetController extends Controller {
    ...
    private function _add() {
        ...
        if (isset($_POST['commit'])) {
            ...
            redirect(getUrl('snippet/edit/' . $snippet->id));
        }
    }
    ...
    private function _edit($id) {
        ...
        if (isset($_POST['csrf_token'])) {
            ...
            if (!SecureToken::validateToken($csrf_token, BASE_URL . 'snippet/edit')) {
                ...
                Flash::set('error', __('Invalid CSRF token found!'));
                Observer::notify('csrf_token_invalid', AuthUser::getUserName());
                redirect(getUrl('snippet/edit/' . $id));
            }
        } else {
            Flash::set('post_data', (object) $data);
            Flash::set('error', __('No CSRF token found!'));
        ...
        if (!$snippet->save()) {
            Flash::set('post_data', (object) $data);
            Flash::set('error', __('Snippet :name has not been saved. Name must be unique!', [':name' => $snippet->name]));
            redirect(getUrl('snippet/edit/' . $id));
        } else {
            Flash::set('success', __('Snippet :name has been saved!', [':name' => $snippet->name]));
    ...
    public function delete($id) {
        ...
        if ($snippet = Record::findByIdFrom('Snippet', $id)) {
            if ($snippet->delete()) {
                Flash::set('success', __('Snippet :name has been deleted!', [':name' => $snippet->name]));
                Observer::notify('snippet_after_delete', $snippet);
            } else {
                Flash::set('error', __('Snippet :name has not been deleted!', [':name' => $snippet->name]));
            }
        } else {
            Flash::set('error', __('Snippet not found!'));
        }
