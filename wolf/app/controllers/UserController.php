...
class UserController extends Controller {
    ...
    private function _add() {
        ...
        if (!empty($data['name']) && !Validate::alphanum_space($data['name'], true)) {
            $errors[] = __('Illegal value for :fieldname field!', [':fieldname' => 'name']);
        }

        if (!empty($data['email']) && !Validate::email($data['email'])) {
            $errors[] = __('Illegal value for :fieldname field!', [':fieldname' => 'email']);
            // Reset to prevent XSS
            $data['email'] = '';
        }

        if (!empty($data['language']) && !Validate::alpha_dash($data['language'])) {
            $errors[] = __('Illegal value for :fieldname field!', [':fieldname' => 'language']);
            // Reset to prevent XSS
        ...
        if (Record::existsIn('User', 'username=:username', [':username' => $data['username']])) {
            $errors[] = __('Username <b>:username</b> is already in use, please choose other!', [':username' => $data['username']]);
        }
    ...
    private function _edit($id) {
        ...
        if ($user->save()) {
            if (AuthUser::hasPermission('user_edit')) {
                ...
                $data = isset($_POST['user_role']) ? $_POST['user_role'] : [];
        ...
        if ($id > 1) {
            ...
            if ($user = Record::findByIdFrom('User', $id)) {
                if ($user->delete()) {
                    ...
                    UserRole::setRolesFor($user->id, []);
                    Flash::set('success', __('User <strong>:name</strong> has been deleted!', [':name' => $user->name]));
                    Observer::notify('user_after_delete', $user->name, $user->id);
                } else {
                    Flash::set('error', __('User <strong>:name</strong> has not been deleted!', [':name' => $user->name]));
                }
            } else {
                Flash::set('error', __('User not found!'));
            }
