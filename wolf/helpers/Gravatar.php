...
class Gravatar {
     ...
     * @return string
     */
    public static function img(string $email, array $attr = [], $size = false, $default = false, $rating = false, $secure = false) {
        $opt = [];
        if ($size !== false)
            $opt['size'] = $size;
        if ($default !== false)
     ...
     * @return type
     */
    public static function profile(string $email, string  $format = 'json') {
        return self::url($email, 'profile', ['format' => $format]);
    }
     ...
     */
    public static function url(string $email, string $type = 'image', array $opt = []) {
        $id = self::hash($email);
